...
class Shape
{
	...
	public:
		...
		virtual void color(Color c) { hue = c; }//Color getter
...
{
	private:
	    Shape** list; // Group's data
	    int numberofShapes;
	public:
	    Group(Color c, int n, Shape** l);  // initializes data
	    ~Group(); // deletes the allocated shapes in the group
/*check*/	    int shapes() const {return numberofShapes;} // returns the number of shapes
/*check*/	    void shapes(int n, Shape** l); // delete the exist data, and change data
/*check*/	    Shape* shape(int n){return list[n];} // returns a pointer to a shape
	    
	    // virtual functions
/*check*/	    void color(Color c);//change all the shape color to the group color
/*check*/	    double area() const; //sum up the area
/*check*/	    double perimeter() const;// similar to area
/*check*/	    void move(double x, double y);//move all the coordinates
	    bool inside (double x, double y) const;//return yes when the point is inside at least one Shapes
